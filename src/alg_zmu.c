...
static void zmu_core(int lid, int lmaxId, int rmaxId, int *ldegree, int *rdegree, int **lrela, int **rrela, double rate, double *lvltr, double *rvltr) {
	int i, j, neigh;

	//one
	memset(rvltr, 0, (rmaxId + 1) * sizeof(double));
	double powl = pow(ldegree[lid], rate);
	...
	for (i = 0; i < rmaxId + 1; ++i) {
		if (rvltr[i]) {
			for (j=0; j<rdegree[i]; ++j) {
				neigh = rrela[i][j];
				double powl = pow(ldegree[neigh], rate);
	...
	for ( i= 0; i < lmaxId + 1; ++i) {
		if (lvltr[i]) {
			double powl = pow(ldegree[i], rate);
...
METRICS *zmu(BIP *train, BIP *test, NET *trainr_cosine_similarity, int num_toprightused2cmptmetrics, double rate_zmuparam, int *l) {
//struct METRICS *hybrid(struct TASK *task) {
	...
	for (i = 0; i<trainl->maxId + 1; ++i) {
		if (trainl->degree[i]) {//each valid user in trainset.
			//get rvlts
			zmu_core(i, lmaxId, rmaxId, ldegree, rdegree, lrela, rrela, rate, lvltr, rvltr);
