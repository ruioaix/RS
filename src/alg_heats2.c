...
static void heats2_core(int lid, int lmaxId, int rmaxId, int *ldegree, int *rdegree, int **lrela, int **rrela, double *lvltr, double *rvltr, double rate_huparam, double rate_hiparam) {
	int neigh, i, j;
	//one
	...
	for (i=0; i<lmaxId + 1; ++i) {
		if (ldegree[i]) {
			double powl = pow(ldegree[i], rate_huparam);
	...
	for (i=0; i<rmaxId + 1; ++i) {
		if (rdegree[i]) {
			double powl = pow(rdegree[i], rate_hiparam);
...
METRICS *heats2(BIP *train, BIP *test, NET *trainr_cosine_similarity, int num_toprightused2cmptmetrics, int *l, double rate_huparam, double rate_hiparam) {
	...
	for (i = 0; i<trainl->maxId + 1; ++i) {
		if (trainl->degree[i]) {//each valid user in trainset.
			//get rvlts
			heats2_core(i, lmaxId, rmaxId, ldegree, rdegree, lrela, rrela, lvltr, rvltr, rate_huparam, rate_hiparam);
