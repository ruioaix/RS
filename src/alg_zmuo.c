...
static void zmuo_core(int lid, int lmaxId, int rmaxId, int *ldegree, int *rdegree, int **lrela, int **rrela, double rate, double *lvltr, double *rvltr) {
	int i, j, neigh;

	//one
	memset(rvltr, 0, (rmaxId + 1) * sizeof(double));
	double powl = pow(ldegree[lid], 1 - rate);
...
//struct METRICS *hybrid(struct TASK *task) {
	...
	for (i = 0; i<trainl->maxId + 1; ++i) {
		if (trainl->degree[i]) {//each valid user in trainset.
			//get rvlts
			zmuo_core(i, lmaxId, rmaxId, ldegree, rdegree, lrela, rrela, rate, lvltr, rvltr);
