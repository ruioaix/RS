...
static void set_maxId_minId_BIP(int *i1, int *i2, long edgesNum, enum SIDE s, int *maxId, int *minId) {
	//set minId&maxId.
	long i;
	if (LEFT == s) {
		*minId = *maxId = i1[0];
...
static void set_degree_idNum_BIP(int *i1, int *i2, long edgesNum, enum SIDE s, int *degree) {
	int i;
	if (LEFT == s) {
		for(i=0; i<edgesNum; ++i) {
			++degree[i1[i]];
...
static void prepare_edges_attr_and_set_degreeMaxMin_idNum_create_Bip(int *i3, double *d1, int maxId, int *degree, int *idNum_retn, int *degreeMax_retn, int *degreeMin_retn, int ***edges_retn, double ***edgesD_retn) {
...
BIP *createBIP(const struct LineFile * const lf, enum SIDE s) {
	...
	int maxId, minId;
	set_maxid_minid_BIP(i1, i2, edgesNum, s, &maxId, &minId);

	int idNum, *degree = scalloc(maxId+1, sizeof(int));
	set_degree_idNum_BIP(i1, i2, edgesNum, s, degree);

	degree = set_degree_create_Bip(i1, i2, Bip->maxId, lf->linesNum, s);

	//set idNum, degreeMax, degreeMin
	prepare_edges_attr_and_set_degreeMaxMin_idNum_create_Bip(lf->i3, lf->d1, Bip->maxId, Bip->degree, &(Bip->idNum), &(Bip->degreeMax), &(Bip->degreeMin), &(Bip->rela), &(Bip->aler));
