...
static void hybridr_core(int lid, int lmaxId, int rmaxId, int *ldegree, int *rdegree, int **lrela, int **rrela, double rate, double *lvltr, double *rvltr) {
	int i, j, neigh;
	...
	for ( i= 0; i < rmaxId + 1; ++i) {
		if (rdegree[i]) {
			for (j = 0; j < rdegree[i]; ++j) {
				neigh = rrela[i][j];
				double powl = pow(ldegree[neigh], 1-rate);
				rvltr[i] += lvltr[neigh]/powl;
			}
			rvltr[i] /= rdegree[i];
		}
	}
...
//struct METRICS *hybrid(struct TASK *task) {
	...
	for (i = 0; i<trainl->maxId + 1; ++i) {
		if (trainl->degree[i]) {//each valid user in trainset.
			//get rvlts
			hybridr_core(i, lmaxId, rmaxId, ldegree, rdegree, lrela, rrela, rate, lvltr, rvltr);
